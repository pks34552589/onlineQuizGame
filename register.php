...
if($_SERVER['REQUEST_METHOD']=="POST") {
    ...
    if(!$nameOk) {
        ...
        $sql = "SELECT * FROM class_".$class." WHERE mobile='$mobile' AND email='$email'";
        $result = mysqli_query($con,$sql);
        $num = mysqli_num_rows($result);
        if($num > 0) {
            $message = "User Already Exists";
        }else {
            $sql = "INSERT INTO class_".$class."(mobile,email,name,school,city,state,dateofreg) VALUES('$mobile','$email','$name','$school','$city','$state','$date')";
            $result = mysqli_query($con,$sql);
            $sql="INSERT INTO class_".$class."_results VALUES('$mobile',-1,-1,-1,-1,1)";
            $result = mysqli_query($con,$sql);
            $_SESSION['mobile'] = $mobile;
            $_SESSION['email'] = $email;
            $_SESSION['name'] = $name;
            $_SESSION['class'] = "class_".$class;
